...
function getRectangleString(width, height) {
  const whiteSpace = ' ';
  const strHead = `┌${'─'.repeat(width - 2)}┐\n`;
  const strMid = `│${whiteSpace.repeat(width - 2)}│\n`;
  const strTail = `└${'─'.repeat(width - 2)}┘\n`;
  return strHead + strMid.repeat(height - 2) + strTail;
}
